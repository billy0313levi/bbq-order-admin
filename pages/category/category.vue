<template>
	<view class="page-body">
		...
		<vk-data-table ref="table1" :action="table1.action" :columns="table1.columns" :query-form-param="queryForm1"
			:right-btns="['update','delete']" :selection="true" :row-no="true" :pagination="true"
			:custom-right-btns="[
					{
						title: '估清',
				...
				]" @custom-right-btns="customRightBtns" @update="updateBtn" @delete="deleteBtn" @current-change="currentChange"
	...
	export default {
		data() {
			...
			return {
				...
				table1: {
					// 表格数据请求地址
					action: "admin/object/sys.category.kh_getList",
					// 表格字段显示规则
					columns: [{
							key: "categoryLogo",
							title: "图片",
				...
				form1: {
					...
					props: {
						...
						columns: [{
							...
							// 	type: "remote-select",
							// 	placeholder: "请选择门店",
							// 	action: "admin/store/store.kh_getList",
							// 	props: {
							// 		list: "rows",
							// 		value: "_id",
							// 		label: "storeName"
		...
		methods: {
			...
			customRightBtns(row, btn) {
				if (btn.title === "估清") {
					this.$confirm('此操作将该类目下所有菜品添加到沽清列表, 是否继续?', '提示', {
						confirmButtonText: '确定',
						cancelButtonText: '取消',
						type: 'warning'
					}).then(() => {
						vk.callFunction({
							url: 'admin/object/sys.category.sys_addGoodsUnderCategoryToestimate',
							title: '请求中...',
							data: {
								_id: row._id
							},
			...
			addBtn() {
				that.resetForm();
				that.form1.props.action = 'admin/object/sys.category.add';
			...
			updateBtn({
				...
				that.form1.props.action = 'admin/object/sys.category.edit';
			...
			deleteBtn({
				...
				deleteFn({
					action: "admin/object/sys.category.del",
					data: {
						_id: item._id
					},
				});
