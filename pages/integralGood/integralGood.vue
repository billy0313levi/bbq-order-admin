	...
	export default {
		data() {
			...
			return {
				...
				table1: {
					// 表格数据请求地址
					action: "admin/object/sys.integralGood.kh_getList",
					// 表格字段显示规则
					columns: [{
							key: "image",
							title: "缩略图",
		...
		methods: {
			...
			addBtn() {
				that.resetForm();
				that.form1.props.action = 'admin/object/sys.integralGood.add';
			...
			updateBtn({
				...
				that.form1.props.action = 'admin/object/sys.integralGood.edit';
			...
			deleteBtn({
				...
				deleteFn({
					action: "admin/object/sys.integralGood.del",
					data: {
						_id: item._id
					},
				});
