<template>
	<view class="page-body">
		...
		<vk-data-table ref="table1" :action="table1.action" :columns="table1.columns" :query-form-param="queryForm1"
			:right-btns="['delete']" :selection="true" :row-no="true" :pagination="true"
	...
	export default {
		data() {
			...
			return {
				...
				table1: {
					// 表格数据请求地址
					action: "admin/object/sys.estimate.kh_getList",
					// 表格字段显示规则
					columns: [{
							key: "logo",
							title: "图片",
		...
		methods: {
			...
			addBtn() {
				that.resetForm();
				that.form1.props.action = 'admin/object/sys.estimate.add';
			...
			deleteBtn({
				...
				deleteFn({
					action: "admin/object/sys.estimate.del",
					data: {
						_id: item._id
					},
				});
