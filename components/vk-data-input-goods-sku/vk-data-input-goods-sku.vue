			...
			<el-table class="sku-list-table" :data="skuListCom" border :header-cell-style="{'text-align':'center'}" :cell-style="{'text-align':'center'}">
				<el-table-column v-for="(item,index) in tableTitleCom" :key="index" :label="item.name">
					<template slot-scope="scope">
						...
						<!-- 是否默认 -->
						<view v-else-if="item.key === 'isDefault'">
							<el-checkbox v-model="scope.row[item.key]" @input="isDefaultChange(scope.$index)"></el-checkbox>
						</view>
						<!-- 是否禁用 -->
						<view v-else-if="item.key === 'disabled'">
							<el-checkbox v-model="scope.row[item.key]" @input="disabledChange" :disabled="offs"></el-checkbox>
						</view>
						<!-- <view>
							<el-input v-model="specListCom[0].list.length"></el-input>
						</view> -->
					</template>
				</el-table-column>
			</el-table>
...
export default {
	...
	methods: {
		...
		specRemove2(item, index) {
			...
			if(this.skuListCom.length > 0 && this.skuListCom.length != 1) {
				if(this.skuListCom[index+1] != null) this.skuListCom[index+1].isDefault = true
				else this.skuListCom[index-1].isDefault = true
			}
			console.log(this.skuListCom)
		},
		...
		isDefaultChange(e){
			this.skuListCom.map((item,index) => {
				console.log(item.isDefault)
				item.isDefault = (e == index) ? true : false
			});
			// this.skuListCom.reverse().reverse();
		},
		// 使input输入框获得焦点
		...
		calcSkuList() {
				...
				for (let i = 0; i < allArr.length; i++) {
					...
					if (needPush) {
						newSkuList.push({
							...
							vipPrice: "", // sku市场价
							// weight: "", // sku重量
							isDefault: newSkuList.length > 0 ? false : true, // 是否默认
							disabled: false, // 是否禁用
						});
					}
				}
			...
			console.log(skuListCom)
			if(skuListCom.length > 1) this.offs = false
			else this.offs = true
	...
	computed: {
		...
		tableTitleCom() {
			...
			let defaultArr = [
				...
				{ key: "isDefault", name: textdataFn("isDefault","是否默认") },
		...
		// offs(){
		// 	return this.specListCom[0].list.lengthh > 1 ? false : true
		// }
	}
};
